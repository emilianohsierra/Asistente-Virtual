...
export default async (req) => {
  try {
    // 1) Leer lo que mandó el frontend (mensaje del cliente, idioma, tono)
    const { mensaje, idioma, tono } = await req.json();
...
Redacta una respuesta breve, clara y con tono ${tono}, escrita en ${idiomaTexto}, para el mensaje de un cliente.
